...
from property_valuation.helpers import process_postal_codes, plot_normal_distribution, format_currency, call_api, build_where_clause, generate_key

def render_property_valuation_tab():
    ...
    postal_codes_input_text = st.text_input("Enter postal codes separated by commas (e.g., 00740, 02320)", key = generate_key("postal_codes_input_text"))
    ...
    selected_built_year_range = st.slider(
        ...
        key = generate_key("selected_built_year_range"),
    )
    ...
    selected_square_meter_range = st.slider(
        ...
        step=1,
        label_visibility="hidden",
        key = generate_key("selected_square_meter_range"),
    )
    ...
    with col1:
        own = st.checkbox("Own", key = generate_key("Own"),)
    with col2:
        rented = st.checkbox("Rented (Leased)", key = generate_key("rented"),)
    with col3:
        unknown = st.checkbox("Unknown", key = generate_key("unknown"),)
    ...
    # Add checkboxes in each column
    with col1:
        one = st.checkbox("Studio (Yksiö)", key=generate_key("Studio"), )
    with col2:
        two = st.checkbox("2 (Kaksiot)", key=generate_key("Kaksiot"), )
    with col3:
        three = st.checkbox("3 (Kolmiot)", key=generate_key("Kolmiot"), )
    with col4:
        four = st.checkbox("4 or more", key=generate_key("4-or-more"), )
    ...
    # Add checkboxes in each column
    with col1:
        kerostalo = st.checkbox("Apartment Building (Kerostalo)", key = generate_key("Kerostalo"),)
    with col2:
        rivitalo = st.checkbox("Row House (Rivitalo)", key = generate_key("Rivitalo"),)
    with col3:
        omakotitalo = st.checkbox("Own Home (Omakotitalo)", key = generate_key("Omakotitalo"),)
    ...
    # Add checkboxes in each column
    with col1:
        good = st.checkbox("Good", key = generate_key("state-Good"),)
    with col2:
        ok = st.checkbox("Satisfying", key = generate_key("state-Satisfying"),)
    with col3:
        bad = st.checkbox("Passable", key = generate_key("state-Passable"),)
